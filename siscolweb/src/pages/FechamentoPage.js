...
useEffect(() => {
    if (formData.polo) {
        ...
        setMunicipios([]); // Limpa o estado se não houver polo selecionado
    }
}, [formData.polo]);
...
useEffect(() => {
    if (formData.setorAbastecimento) {
        ...
        setZonasPressao([]); // Limpa o estado se não houver setor selecionado
    }
    ...
    if (formData.microzona === 'Sim') {
        requiredFields.push('numeroMZ'); // 'Número MZ' é obrigatório se 'Possui Microzona' é 'Sim'
    } else if (formData.microzona === 'Não') {
        requiredFields.push('motivo'); // 'Motivo' é obrigatório se 'Possui Microzona' é 'Não'
    }
